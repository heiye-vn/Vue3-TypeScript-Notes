...
const arr_colors: string[] = new Array("red", "blue", "yellow", "gray");
...
// 数组迭代
let arr_obj: object[] = [{name: "张三", age: 10}, {name: "李四", age: 20}, {name: "王五", age: 30}];
for (let item of arr_obj) {
    // console.log(item, "===========", typeof item);
}

const arr_animal: string[] = ["cat", "tiger", "lion", "elephant", "mouse"];
let j: string;
for (j in arr_animal) {
...
let arr_multi: number[][] = [[1, 2, 3], [4, 5, 6], [7, 8, 9]];
...
// 作为参数传给函数
let sites: string[] = ["Google", "Taobao", "Facebook"];

function printSite(arr_sites: string[]) {
...
// 作为函数的返回值
function test_fun(): string[] {
    return ["张三", "李四", "王五"];
}
let arr_person: string[] = test_fun()
